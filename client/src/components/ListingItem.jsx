    ...
    <div className='bg-whitecream shadow-md sm:shadow-lg transition-shadow overflow-hidden rounded-lg w-full sm:w-[330px]'>
        ...
        <div className='p-3 flex flex-col gap-2 w-full'>
            ...
            <div className='flex items-center gap-1'>
                <MdLocationOn className='h-4 w-4 text-brownish'/>
                <p className='text-sm text-brownish truncate'>{listing.address}</p>
            </div>
            <p className='text-sm text-brownish line-clamp-2'>{listing.description}</p>
            <p className='text-brownish mt-2 font-semibold flex items-center'>
                Rs. {listing.offer ? listing.discountPrice.toLocaleString('en-US') : listing.regularPrice.toLocaleString('en-US')} 
                {listing.type === 'rent' && ' / month'}
            </p>
            <div className='flex gap-4 text-brownish'>
